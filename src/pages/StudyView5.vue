...
export default {
  setup() {
    ...
    const login = () => {      
      axios.post("http://localhost:8080/api/account/login", state.form, {withCredentials: true}).then((res) => {               
    ...
    const logout = () => {      
      axios.get("http://localhost:8080/api/account/logout", {withCredentials: true}).then((res) => {
        window.alert(res.data);
      }).catch(() => {
    ...
    const check = () => {      
      axios.get("http://localhost:8080/api/account/check", {withCredentials: true}).then((res) => {       
        window.alert(res.data);
      });
    }
